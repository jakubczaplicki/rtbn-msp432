// ADCTestMain.c
// Runs on MSP432
// This program periodically samples ADC channels 6 and 7 and
// stores the results to global variables that can be
// accessed with the JTAG debugger and viewed with the
...
int main(void){
  ADC0_InitSWTriggerCh67();        // initialize ADC sample P4.7/A6 and P4.6/A7
  ...
  while(1){
    ...
    P2OUT = (P2OUT&~0x07)|(COLORWHEEL[(Ch6>>11)&(WHEELSIZE-1)]);
    // update the single LED according to the ADC measurement
    P1OUT = (P1OUT&~0x01)|((Ch7>>13)&0x01);
    WaitForInterrupt();
  }
}
