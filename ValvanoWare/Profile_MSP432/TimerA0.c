...
void TimerA0_Init(void(*task)(void), uint16_t period){long sr;
  ...
  TA0CCTL0 = (TA0CCTL0&~0xC105)|0x0010;
  TA0CCR0 = (period - 1);          // compare match value
  TA0EX0 &= ~0x0007;               // configure for input clock divider /1
  NVIC_IPR2 = (NVIC_IPR2&0xFFFFFF00)|0x00000060; // priority 3
// interrupts enabled in the main program after all devices initialized
  NVIC_ISER0 = 0x00000100;         // enable interrupt 8 in NVIC
  TA0CTL |= 0x0014;                // reset and start Timer A0
  EndCritical(sr);
}

void TA0_0_IRQHandler(void){
  TA0CTL &= ~0x0001;               // acknowledge Timer A0 timeout
  if(TA0CCTL0&0x0001){             // poll TA0 CCTL0
