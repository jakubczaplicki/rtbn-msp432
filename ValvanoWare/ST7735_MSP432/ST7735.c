...
uint8_t static writecommand(uint8_t c) {
  ...
  DC &= ~DC_BIT;
  UCA3TXBUF = c;                        // command out
  while((UCA3IFG&0x0001)==0x0000){};    // wait until UCA3RXBUF full
  return UCA3RXBUF;                     // return the response
}
...
void static commonInit(const uint8_t *cmdList) {
  ...
  P9SEL0 &= ~0x1C;
  P9SEL1 &= ~0x1C;                      // configure P9.2 (D/C), P9.3 (Reset), and P9.4 (TFT_CS) as GPIO
  ...
  P9SEL0 &= ~0x0C;
  P9SEL1 &= ~0x0C;                      // configure P9.3 and P9.2 as GPIO (Reset and D/C pins)
  P9DIR |= 0x0C;                        // make P9.3 and P9.2 out (Reset and D/C pins)
