...
// Last modified 7/10/13 by Jonathan W. Valvano
...
1) Create a bmp file
   ...
   store in same directory as BmpConvert16-bit.exe
...
//exact copy code for debug purposes
  ...
  if(bmpWidth > 160){
    fprintf(stderr, "Warning: This image's width of %d exceeds the ST7735's 160 pixel width.\n", bmpWidth);
  }
  ...
  if(bmpHeight > 128){
    fprintf(stderr, "Warning: This image's height of %d exceeds the ST7735's 128 pixel height.\n", bmpHeight);
  }
  if(bmpHeight < 0){
  ...
  for(i=1; i<=(bmpDataSize/3); i=i+1){
    blue = fgetc(in);
    green = fgetc(in);
    red = fgetc(in);
    color = ((blue & 0xF8) << 8) | ((green & 0xFC) << 3) | (red >> 3);
    fputc(' ', out);
    fputc('0', out);
    fputc('x', out);
    outUHex(color, out);
