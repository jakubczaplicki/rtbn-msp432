...
// Joystick:
//   Continue measuring analog on P6.0 and P4.4 and digital on P4.1.
//   The user may connect external potentiometers and a switch for
//   the educational value, but the software will continue to work
//   regardless.  Wire the switch to be zero when pressed.
// RGB LED:
//   Use the RGB LED on the LaunchPad.
// Buzzer:
//   Continue to output a square wave on P2.7.  The user may connect
//   external headphones through a 1k resistor for the educational
//   value, but the software will continue to work regardless.
// Accelerometer:
//   Continue measuring analog on P6.1, P4.0, and P4.2.  The user
//   may connect external potentiometers for the educational value,
//   but the software will continue to work regardless.
// Microphone:
//   Continue measuring analog on P4.3.  The user may connect an
//   external potentiometer for the educational value, but the
...
//   Send ASCII characters through eUSCIA0 to a computer at 115,200
...
// 1 to 6 potentiometers
// 1 1,000 ohm resistor, any tolerance, any wattage
// 1 female headphone jack
// 1 32-ohm headphones or ear buds
// 1 momentary switch
// 1 10,000 ohm resistor, any tolerance, any wattage
// many solid wires
...
#define BUTTON1   (*((volatile uint8_t *)(0x42000000+32*0x4C00+4*1)))  /* Port 1.1 Input */
...
void BSP_Button2_Init(void){
  P1SEL0 &= ~0x10;
  P1SEL1 &= ~0x10;                 // configure P1.4 as GPIO
  P1DIR &= ~0x10;                  // make P1.4 in
  P1REN |= 0x10;                   // enable pull resistor on P1.4
  P1OUT |= 0x10;                   // P1.4 is pull-up
}
...
#define BUTTON2   (*((volatile uint8_t *)(0x42000000+32*0x4C00+4*4)))  /* Port 1.4 Input */
...
void BSP_RGB_Init(uint16_t red, uint16_t green, uint16_t blue){
  ...
  PMAPKEYID = 0x2D52;              // unlock Port Mapping Controller
  PMAPCTL |= 0x0002;               // allow reconfiguration of port mapping
                                   // configure P2.0 for TA0.3
                                   // configure P2.1 for TA0.1
  P2MAP01 = (PM_TA0CCR1A<<8)|PM_TA0CCR3A;
                                   // configure P2.2 for TA0.2
  P2MAP23 = (P2MAP23&~0x00FF)|PM_TA0CCR2A;
  PMAPKEYID = 0;                   // lock Port Mapping Controller
  // ***************** Timer A0 initialization *****************
  // RGB LED green connected to P2.1 on MSP432 LaunchPad, which is mapped to TA0.1
  // RGB LED blue connected to P2.2 on MSP432 LaunchPad, which is mapped to TA0.2
  // RGB LED red connected to P2.0 on MSP432 LaunchPad, which is mapped to TA0.3
  ...
  // bit0=0,           clear capture/compare interrupt pending
  TA0CCTL2 = 0x00E0;
  TA0CCR2 = (blue*PWMCycles)>>10; // defines when output signal is cleared
  // bits15-14=00,     no capture mode
  // bits13-12=XX,     capture/compare input select
  // bit11=X,          synchronize capture source
  // bit10=X,          synchronized capture/compare input
  ...
  // ***************** GPIO initialization *****************
  P2SEL0 |= 0x07;
  P2SEL1 &= ~0x07;                 // configure P2.2-P2.0 as timer out
  P2DIR |= 0x07;                   // make P2.2-P2.0 out
  P2DS |= 0x07;                    // enable high drive strength on P2.2-P2.0
}
...
// stand alone section
  ...
  light = 100*LightCalls;          // arbitrary return value
// end of stand alone section
  LightBusy = 0;
  return light;
}
...
// stand alone section
      LightCalls = LightCalls + 1;
      lightLocal = 100*LightCalls; // arbitrary return value
// end of stand alone section
      *light = lightLocal;
      LightBusy = 0;
...
// stand alone section
  ...
  volt = 10000*TempCalls;          // arbitrary return value
  temp = 10000*TempCalls;          // arbitrary return value
// end of stand alone section
  *sensorV = volt;
  *localT = temp;
  TempBusy = 0;
